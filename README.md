...
<details>
  <summary>Context switching이 무엇인가요?</summary>

프로세스가 수행되는 CPU에 다른 프로세스를 실행하기 위해 이전 프로세스의 문맥을 메모리에 저장하고 새로운 프로세스의 PCB를 참고하여 레지스터리에 적재하는 과정입니다.
인터럽트가 발생하는 경우, 프로세스의 타임 슬라이스를 모두 소진한 경우 컨텍스트 스위칭이 발생합니다.

> 스레드 컨텍스트 스위칭을 할때 어떤 오버헤드가 존재하나요?

- 동일한 프로세스의 다른 스레드로 스위칭 하는 경우
    - 현재 CPU 상태를 TCB에 저장하고 새 스레드의 TCB 값을 레지스터에 적재하는 시간
    - TCB 리스트 조작 시간(준비리스트나 블록리스트 등으로 옮기는 시간)
- 다른 프로세스의 스레드로 스위칭되는 경우
    - 이전 페이지 테이블을 제거하고 새로운 페이지 테이블 적재 및 TLB 새로 채움
...
<details>
  <summary>메모리는 왜 운영체제에 의해 관리되나요?</summary>

- 메모리는 여러 프로세스에 의해 사용되는 공유 자원, 프로세스가 임의로 사용하는 것이 아니라 운영체제가 할당된 영역과 비어있는 영역을 관리할 필요
- 메모리 보호(프로세스의 메모리를 다른 프로세스가 접근하지 못하도록, 사용자모드일 때 커널 공간 접근 못하도록)
- 메모리 용량 한계 극복(가상메모리)
- 메모리 사용의 효율 극대화, 정해진 메모리 용량에서 가능한 많은 프로세스들을 실행

→ 커널 모드와 사용자 모드에 대해서 설명해주세요

커널 모드에서 CPU는 모든 메모리 공간을 액세스할 수 있고 특권 명령을 실행할 수 있으며, 어떤 하드웨어든지 접근하고 제어할 수 있습니다.

반면, 사용자 모드에서는 사용자 공간만 접근할 수 있고 커널 공간은 접근할 수 없습니다. 그리고 특권명령으로 불리는 기계 명령들을 실행할 수 없습니다. 특권 명령은 입출력 장치나 타이머, 인터럽트 처리, 시스템 중단 등의 특수한 CPU 명령입니다.

</details>

<details>
  <summary>32bit CPU에서는 4GB 초과의 메모리를 장착해도 인식할 수 없는데 그 이유가 무엇인가요?</summary>

32bit CPU에서는 주소선이 32개이기 때문입니다. 이는 CPU가 어떻게 특정 주소에 있는 데이터를 접근하는지 알아야 합니다.

CPU가 32개의 주소선을 통해 주소를 출력하면 32비트의 주소가 주소버스를 통해 메모리에게 전달됩니다. 메모리 읽기의 경우 메모리가 주소를 전달 받고 일정 시간 후에 해당 주소의 데이터를 데이터 버스에 내놓습니다. 그러면 CPU는 자신의 데이터 선을 통해 데이터 값을 CPU 내부로 읽어 들입니다.

그렇기 때문에 32비트 CPU의 경우 액세스할 수 있는 주소의 범위는 0~(2^32-1)번지 입니다. 왜냐하면 32비트로 표현할 수 있는 최대 정수는 2^32-1이기 때문입니다.

또한 한 번지의 크기는 1바이트이므로, 32비트 CPU가 액세스할 수 있는 메모리의 최대 범위는 (2^32)Byte = 4 x 1GB, 즉 4GB가 됩니다.

</details>

<details>
  <summary>MMU란?</summary>

CPU 칩 내부에 구성된 주소변환 하드웨어입니다. MMU는 프로세스의 논리 주소와 물리 주소의 매핑 정보를 담은 매핑 테이블을 참고하여 논리 주소를 물리 주소로 변환합니다.

→ 운영체제는 왜 논리주소와 물리주소를 나누어 관리할까요?

- 한정된 메모리를 효율적으로 사용할 수 있기 때문입니다. 예를 들어 가상 메모리가 있습니다.
- 메모리 보호입니다. 프로세스가 다른 프로세스의 메모리 영역을 침범하지 못하도록 합니다.
- 메모리 관리 단순화입니다. 개발자 입장에서 논리 주소로 개발하면 되기 때문에 물리 주소를 신경쓰지 않고 개발할 수 있습니다.
